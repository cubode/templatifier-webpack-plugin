...
class TemplatifierWebpackPlugin {
    ...
    apply(compiler) {
        compiler.hooks.beforeCompile.tapAsync('TemplatifierWebpackPlugin', (params, callback) => {
            ...
            html = html.replace(/<!--\s*<cb-templatifier type="static-css" route="([^"]+)"(?: name="([^"]*)")?(?: version="([^"]*)")?\s*\/>\s*-->/g, (match, route, name, version) => {
                version = version ? `?v={{ ${version} }}` : '';
                if (name) {
                    return `<link href="{% ${name} '${route}' %}${version}" rel="stylesheet">`;
                } else {
                    return `<link href="{% static '${route}' %}${version}" rel="stylesheet">`;
                }
            });

            // Process the <cb-templatifier> elements for static-script
            html = html.replace(/<!--\s*<cb-templatifier type="static-script" route="([^"]+)"(?: name="([^"]*)")?(?: version="([^"]*)")?\s*\/>\s*-->/g, (match, route, name, version) => {
                version = version ? `?v={{ ${version} }}` : '';
                if (name) {
                    return `<script defer="defer" src="{% ${name} '${route}' %}${version}"></script>`;
                } else {
                    return `<script defer="defer" src="{% static '${route}' %}${version}"></script>`;
                }
            });
